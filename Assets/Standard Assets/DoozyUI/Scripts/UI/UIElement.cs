...
namespace DoozyUI
{
    ...
    public class UIElement : MonoBehaviour
    {
        ...
        static void CreateElement(UnityEditor.MenuCommand menuCommand)
        {
            ...
            Canvas targetCanvas = selectedGO.GetComponent<Canvas>(); ;
        ...
        public void Show(bool instantAction)
        {
            ...
            {
                Debug.LogWarning("[DoozyUI] [" + name + "] You are trying to SHOW the  UIElement, but you didn't enable any IN animations. To fix this warning you should enable at least one IN animation.");
                return;
            }
        ...
        public void Hide(bool instantAction, bool shouldDisable)
        {
            ...
            {
                Debug.LogWarning("[DoozyUI] [" + name + "] You are trying to HIDE the UIElement, but you didn't enable any OUT animations. To fix this warning you should enable at least one OUT animation.");
                return;
            }

            if(isVisible)
            {
                if(!instantAction) { TriggerOutAnimationsEvents(instantAction); } //we do this check so that the events are not triggered onEnable when we have startHidden set as true
                else
                {
                    if (useBackground && bg != null && bg.isActiveAndEnabled)
                    {
                        UIManager.Instance.HideBackgroundPanel(bg, instantAction);
                    }
        ...
        private void TriggerInAnimationsEvents(bool instantAction)
        {
            if (useBackground)
            {
                bg = UIManager.Instance.GetBackgroundPanel(instantAction);
                bg.Canvas.sortingOrder = this.Canvas.sortingOrder - 1;
            }
        ...
        {
            if (useBackground && bg.isActiveAndEnabled)
            {
                UIManager.Instance.HideBackgroundPanel(bg, instantAction);
            }
